...
update User set expense = expense (expense - 200), balance = balance - (balance) where id = 1;
select * from Category;
select * from Registration where User_id = 1;


select Registration.id, Category.name, Category.type, Registration.value, Registration.data, 
Category.class from Registration join Category on Category_id = Category.id and Registration.User_id = 1
where Category.type = 'D';
